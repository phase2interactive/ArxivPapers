...
def process_line(i, line, dr, args):
    ...
    os.system(
        f'{args.gs} -sDEVICE=pdfwrite -dNOPAUSE -dBATCH -dFirstPage={page_num+1} -dLastPage={page_num+1} -sOutputFile={os.path.join(dr, str(page_num))}.pdf {os.path.join(dr,"main.pdf")} > /dev/null 2>&1'
    )

    # convert to PNG
    os.system(
        f"{args.gs} -sDEVICE=png16m -r300 -o {os.path.join(dr, str(page_num))}.png {os.path.join(dr, str(page_num))}.pdf"
    )
    ...
    os.system(
        f"{args.ffmpeg} -loop 1 -i {os.path.join(dr, str(page_num))}.png -i {os.path.join(dr, audio)}.mp3 "
    ...
    os.system(
        ...
        f"{args.ffmpeg} -i {video_file} -t $audio_duration "
...
def main(args):
    ...
    if os.path.exists(os.path.join(dr, "shorts_mp3_list.txt")):
        ...
        results = pool.starmap(
            process_short_line, [(i, line, page_num, dr, args) for i, (page_num, line) in enumerate(lines)]
        )
        pool.close()
        pool.join()

        results.sort(key=lambda x: x[0])
        with open(os.path.join(dr, "short_mp4_list.txt"), "w") as outvideo:
            outvideo.writelines([r for _, r in results])

        os.system(
            f'{args.ffmpeg} -f concat -i {os.path.join(dr, "short_mp4_list.txt")} '
...
def process_short_line(i, line, page_num, dr, args):
    ...
    # ensure that there is no silence at the end of the video, and video len is the same as audio len
    os.system(
        f"audio_duration=$({args.ffprobe} -i {os.path.join(dr, audio)}.mp3 -show_entries format=duration "
...
def process_qa_line(i, line, line_num, qa_pages, dr, args):
    ...
    resolution = "scale=1920:-2"
    os.system(
        f"{args.ffmpeg} -loop 1 -i {os.path.join(dr, qa_page)} -i {os.path.join(dr, audio)}.mp3 "
        f"-vf {resolution} -c:v libx264 -tune stillimage -y -c:a aac -b:a 128k -pix_fmt yuv420p "
        f"-shortest {os.path.join(dr, video)}.mp4"
    )
